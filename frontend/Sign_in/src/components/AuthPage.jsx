...
function FixedGraduationCap() {
  return (
    <svg width="40" height="40" viewBox="0 0 145 115" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M8 43L72 12L137 43L72 70L8 43Z" fill="#C5A880" stroke="#9A7B4F" strokeWidth="3.5" strokeLinejoin="round" />
      <path d="M38 57V91C57 103 88 103 107 91V57L72 72L38 57Z" fill="#9A7B4F" stroke="#735A36" strokeWidth="3.5" />
      <path d="M123 38V75" stroke="#E2C799" strokeWidth="3.5" strokeLinecap="round" />
    </svg>
  );
}
...
function EyeIcon({ show }) {
  return show ? (
    <svg className="w-4 h-4 text-slate-400 hover:text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.8" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858-5.908a9.04 9.04 0 012.122-.363c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21M3 3l18 18" />
    </svg>
  ) : (
    <svg className="w-4 h-4 text-slate-400 hover:text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function AuthPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      triggerToast("success", data.message || "Login successful! Redirecting...");

      const targetRole = data.user?.role || role;
      setTimeout(() => {
        navigate(targetRole === "mentor" ? "/mentor-dashboard" : "/student-dashboard");
      }, 900);
    } catch (err) {
      triggerToast("error", err.message || "Invalid credentials provided.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSendOtp = async (event) => {
    ...
    if (!resetEmail.toLowerCase().endsWith("@rajalakshmi.edu.in")) {
      setModalStatus({ type: "error", message: "Email must end with @rajalakshmi.edu.in" });
      return;
    }
    ...
    try {
      ...
      setForgotStep(2);
      setModalStatus({ type: "success", message: `An OTP has been sent to ${resetEmail}` });
    } catch {
      setModalStatus({ type: "error", message: "Backend server is not running. Unable to send OTP." });
    } finally {
  ...
  const handleResetPassword = async (event) => {
    ...
    try {
      ...
      setModalStatus({ type: "error", message: "Backend server is not running or invalid OTP." });
    } finally {
      setIsModalLoading(false);
    }
  };
    ...
    <main className="relative flex min-h-screen w-full items-center justify-center bg-[#EBEEF2] p-4 md:p-8 lg:p-12 font-sans">
      ...
      <div
        className={`fixed top-6 left-1/2 -translate-x-1/2 z-50 transition-all duration-500 ease-in-out transform ${
          toast.show ? "translate-y-0 opacity-100 scale-100" : "-translate-y-16 opacity-0 scale-95 pointer-events-none"
        }`}
      >
        <div
          className={`flex items-center space-x-3 px-6 py-3.5 rounded-xl shadow-2xl border text-sm font-semibold max-w-md ${
            toast.type === "success" ? "bg-emerald-700 text-white border-emerald-600" : "bg-red-700 text-white border-red-600"
          }`}
        >
          {toast.type === "error" ? (
          ...
          <button onClick={() => setToast({ ...toast, show: false })} className="ml-auto text-white/80 hover:text-white">
            &times;
          </button>
        </div>
      </div>
      ...
      <div className="w-full max-w-5xl mx-auto flex flex-col my-auto">
        <div className="w-full rounded-2xl bg-white shadow-2xl flex flex-col md:flex-row overflow-hidden border border-slate-200/80">
          ...
          <div className="relative w-full md:w-1/2 bg-[#0B1321] text-white flex flex-col justify-between px-8 md:px-12 lg:px-14 py-10 md:py-12 lg:py-14 min-h-[520px]">
            ...
            <div className="relative z-10 flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-xl font-serif font-semibold tracking-wide text-slate-100">Scholar Portal</h1>
            ...
            <div className="relative z-10 my-auto py-6">
              ...
              <h2 className="text-3xl md:text-4xl font-serif font-normal text-slate-100 leading-tight">Dream believe achieve</h2>
              <p className="text-xs text-slate-400 mt-4 tracking-wider">Learn &rarr; Grow &rarr; Consistent &rarr; Success.</p>
            ...
            <div className="relative z-10 flex items-center space-x-2 text-[11px] tracking-widest text-slate-400 uppercase font-medium">
              <svg className="w-4 h-4 text-amber-600/90" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path
                  strokeLinecap="round"
          ...
          <div className="w-full md:w-1/2 bg-white px-8 md:px-12 lg:px-14 py-10 md:py-12 lg:py-14 flex flex-col justify-between">
            <div>
              ...
              <div className="mb-8">
                <h2 className="text-2xl font-serif text-slate-800 font-normal">Sign in</h2>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  <label htmlFor="email" className="mb-1.5 block text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                  ...
                  <div className="relative flex items-center">
                    ...
                    <input
                      ...
                      placeholder={
                        isMentor ? "Enter faculty email (e.g., name@rajalakshmi.edu.in)" : "Enter student email (e.g., name@rajalakshmi.edu.in)"
                      }
                ...
                <div>
                  <div className="mb-1.5 flex items-center justify-between">
                    ...
                    <button type="button" onClick={() => setIsForgotModalOpen(true)} className="text-xs text-slate-500 hover:text-slate-800 transition">
                      Forgot password?
                    </button>
                  </div>
                  <div className="relative flex items-center">
                    ...
                    />
                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3.5 flex items-center justify-center">
                ...
                >
                  <span>{isLoading ? "Signing in..." : `Sign in as ${role}`}</span>
        ...
        <div className="w-full mt-4 flex flex-col md:flex-row items-center justify-between text-xs text-slate-500 px-2">
          <div className="flex items-center space-x-6">
            ...
            <a href="#terms" className="hover:text-slate-800 transition">Terms of use</a>
            <a href="#privacy" className="hover:text-slate-800 transition">Privacy policy</a>
          </div>
          <span className="mt-2 md:mt-0 text-slate-500">© 2026 Scholar Hub, Inc.</span>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm">
          <div className="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl border border-slate-100">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div>
                <h2 className="text-lg font-serif font-semibold text-slate-800">Reset Password</h2>
                <p className="mt-0.5 text-xs text-slate-500">Recover access to your account</p>
              </div>
              <button type="button" onClick={closeModal} className="text-xl leading-none text-slate-400 hover:text-slate-700 transition">
                &times;
              </button>
            </div>
            ...
            {modalStatus.message && (
              <div
                className={`mt-4 rounded-lg border p-3 text-xs ${
                  modalStatus.type === "success" ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-red-200 bg-red-50 text-red-700"
                }`}
              >
                {modalStatus.message}
              </div>
            ...
            {forgotStep === 1 && (
              <form onSubmit={handleSendOtp} className="mt-4 space-y-4">
                ...
                <div>
                  <label htmlFor="resetEmail" className="mb-1 block text-xs font-medium text-slate-700">
            ...
            {forgotStep === 2 && (
              <form onSubmit={handleResetPassword} className="mt-4 space-y-3">
                <div>
                  <label htmlFor="otp" className="mb-1 block text-xs font-medium text-slate-700">
                    OTP Code
                  </label>
                  <input
                    id="otp"
